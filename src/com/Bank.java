...
public class Bank {
	...
	public static void main(String args[]) throws BankATMException {
		...
		BankAtm bankAtmICICI = new BankAtm();
		BankAtm bankAtmHDFC = new BankAtm();
		BankAtm bankAtmSBI = new BankAtm();
		...
		while (true) {																//	Loop for the Banks choice
			...
			if (bankChoice <= 3) {													//	Condition to check for the valid bank choice
				while (true) {														//	Loop for the Bank transaction choice
					...
					} catch (BankATMException e) {									//	Catch block to handle the Bank Exception and to Continue the Bank flow
						System.out.println(e.errorMessage);							//	Prints the error message of the Bank Exception 
					}
				}
